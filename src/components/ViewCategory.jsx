...
<div className="flex flex-col flex-1 overflow-hidden md:ml-[300px]">
  ...
  <main className="flex-1 bg-gray-50 p-4 sm:p-6 overflow-y-auto">
              ...
              {filteredCategories.map((category) => {
                ...
                return (
                  <tr key={category.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-500 capitalize">{category.type || 'main'}</td>
                    <td className="px-4 py-3 text-gray-500">{parentCategory}</td>
                    <td className="px-4 py-3 text-gray-500">
                      {new Date(category.createdAt).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end space-x-3">
                        <Link to={`/dashboard/edit-category/${category.id}`} className="text-indigo-600 hover:text-indigo-900" title="Edit">
                          <FiEdit2 size={16} />
                        </Link>
    ...
    {sidebarOpen && (
      <div 
        className="fixed inset-0 bg-transparent bg-opacity-50 z-0 backdrop-blur-sm"
        onClick={() => setSidebarOpen(false)}
      />
    )}
    </div>
